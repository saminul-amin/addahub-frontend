...
import { PlusCircle, Calendar, ListTodo, MapPin, User as UserIcon, Mail, Trash2, Users } from "lucide-react";
                            ...
                            <TabsContent value="overview">
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                    <Card>
                                        ...
                                        <CardContent>
                                            <div className="text-2xl font-bold">{allUsers.filter(u => u.role === 'user').length}</div>
                                            <p className="text-xs text-muted-foreground">Registered standard users</p>
                                        </CardContent>
                                    </Card>
                                    <Card>
                                        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                                        ...
                                        <CardContent>
                                            <div className="text-2xl font-bold">{allUsers.filter(u => u.role === 'host').length}</div>
                                            <p className="text-xs text-muted-foreground">Active event organizers</p>
                                        </CardContent>
                                    </Card>
                                    <Card>
                                        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                                        ...
                                        <CardContent>
                                            ...
                                            <p className="text-xs text-muted-foreground">Published on platform</p>
                                        </CardContent>
                                    </Card>
                                </div>
                            </TabsContent>
                             ...
                             <TabsContent value="users">
                                <Card>
                                    <CardHeader>
                                        ...
                                        <CardDescription>View and manage registered users (filtering hosts out).</CardDescription>
                                    </CardHeader>
                                    <CardContent>
                        ...
                        {user?.role === 'admin' && (
                             <TabsContent value="hosts">
                                <Card>
                                    ...
                                    <CardContent>
                                        <div className="space-y-4">
                                            {allUsers.filter(u => u.role === 'host').map(u => (
                                                <div key={u._id} className="flex items-center justify-between p-4 border rounded-lg">
                                                    <div>
                                                        <p className="font-semibold">{u.name}</p>
                                                        <p className="text-sm text-gray-500">{u.email}</p>
                             ...
                             <TabsContent value="manage_events">
                                <Card>
                                    <CardHeader>
                                        ...
                                        <CardDescription>View, edit, or delete events.</CardDescription>
                                    </CardHeader>
                                    <CardContent>
                                        <div className="space-y-4">
                                            {allEvents.map(e => (
                                                <div key={e._id} className="flex items-center justify-between p-4 border rounded-lg">
                                                    ...
                                                    <div className="flex gap-2">
                                                        ...
                                                        <Button variant="ghost" size="icon" className="text-red-500 hover:bg-red-50" onClick={() => handleDeleteEvent(e._id)}>
                                                            <Trash2 className="h-4 w-4" />
                                                        </Button>
                                                    </div>
                                                </div>
                                ...
                                <CardContent>
                                     {hostedEvents.length > 0 ? (
                                        <div className="space-y-4">
                                            {hostedEvents.map(event => (
                                                <div key={event._id} className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 rounded-lg border bg-card hover:bg-accent/50 transition-colors gap-4">
                                                    ...
                                                    <Button variant="outline" size="sm" asChild>
                                                        <Link href={`/events/${event._id}/edit`}>Manage</Link>
                                                    </Button>
                                                </div>
                                            ))}
                                        </div>
                                        ...
                                        <div className="flex flex-col items-center justify-center p-12 text-center bg-gray-50/50 rounded-xl border-2 border-dashed">
                                            ...
                                            {user?.role === 'host' || user?.role === 'admin' ? (
                                                <Button className="bg-indigo-600 hover:bg-indigo-700">Create My First Event</Button>
                                            ) : (
